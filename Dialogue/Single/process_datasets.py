...
def split_dialogue(dataset, f_path=None, split=None):
    ...
    if f_path is not None:
        os.makedirs(f'data/{f_path}', exist_ok=True)
        save_text(f"data/{f_path}/{split}.src", src)
        save_text(f"data/{f_path}/{split}.trg", trg)        
...
def process_daily(split, save=False):
    ...
    orig = [json.loads(line) for line in open(f'data/dailydialog/{split}.json', 'r')]
    processed = []
    ...
    if save:
        ...
        save_json(processed, f"data/daily/{split}.json")        
...
def process_persona(split, save=False):
    ...
    with open(f"data/Persona-Chat/personachat/{split}_self_original.txt") as f:
        orig = f.readlines()
    ...
    if save:
        ...
        save_json(processed, f"data/persona/{split}.json")
...
def process_empathetic(split, save=False):
    ...
    with open(f'data/empatheticdialogues/empatheticdialogues/{split}.csv', 'r') as f:
        orig = f.readlines()
    ...
    if save:
        os.makedirs('data/empathetic', exist_ok=True)
        save_json(processed, f"data/empathetic/{split}.json")
...
def process_blended(split, save=False):
    ...
    with open(f'data/blended_skill_talk/{split}.json', 'r') as f:
        orig = json.load(f)
...
def main():
    ...
    save_text(train_src, 'data/dialogue/train.src')
    save_text(valid_src, 'data/dialogue/valid.src')
    save_text(test_src, 'data/dialogue/test.src')

    save_text(train_trg, 'data/dialogue/train.trg')
    save_text(valid_trg, 'data/dialogue/valid.trg')
    save_text(test_trg, 'data/dialogue/test.trg')
